...
public class UpdateCatalog implements ActionListener{
	JFrame frame;
	JTextField tf,tf1;
	JLabel lb,lb1;
	JButton bt,bt1;
	public UpdateCatalog()
	{
		...
		lb=new JLabel("Previous CatalogId:");
		...
		if(e.getSource()==bt)
		{
			try
			{
				Class.forName("oracle.jdbc.OracleDriver");
			      ...
			      String k=tf.getText();
			      String str="select*from product where catalogid='"+k+"'";
			      ResultSet rs=stmt.executeQuery(str);
			      if(rs.next())
			      {
			      stmt.executeUpdate("update product set catalogid='"+j+"' where catalogid='"+k+"'");
